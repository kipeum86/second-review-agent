...
AI가 만든 법률 문서를 외부 공유 전에 한 번 더 점검하는 KP Legal Orchestrator의 최종 검토 에이전트. Claude Code 기반.
...
`시니어 리뷰 스페셜리스트`는 KP Legal Orchestrator의 최종 검토 에이전트입니다. 계약 검토 스페셜리스트(`contract-review`), 법률 드래프팅 스페셜리스트(`legal-writing`), 법률 리서치 스페셜리스트(`general-legal-research`), 게임 산업법 스페셜리스트(`game-legal-research`)가 만든 문서를 받아서 인용을 검증하고, 논리를 따지고, 문장을 다듬고, 레드라인 DOCX로 결과를 냅니다. 문서가 외부 공유 단계로 넘어가기 전 마지막 관문입니다.

페르소나는 **시니어 리뷰 스페셜리스트** — AI가 만든 문서를 본능적으로 의심하는 회의적인 검토자입니다. 덕분에 워크플로우 안에서 가장 집요하게 검증합니다. 여백에 빨간펜, 한 줄 코멘트, 가짜 판례에는 무관용입니다.
...
**KP Legal Orchestrator**의 전문 법률 워크플로우 에이전트 시리즈:

| 에이전트 | 역할 | 중점 분야 |
|---------|------|----------|
| `game-legal-research` | 게임 산업법 스페셜리스트 | 게임 산업법 |
| `legal-translation-agent` | 법률 번역 스페셜리스트 | 법률 번역 |
| `general-legal-research` | 법률 리서치 스페셜리스트 | 법률 리서치 |
| `PIPA-expert` | 개인정보보호법 스페셜리스트 | 개인정보보호법 |
| `GDPR-expert` | GDPR 스페셜리스트 | 데이터 보호법 (GDPR) |

...
`시니어 리뷰 스페셜리스트`는 KP Legal Orchestrator의 최종 검토 에이전트입니다. 계약 검토 스페셜리스트(`contract-review-agent`), 법률 드래프팅 스페셜리스트(`legal-writing-agent`), 법률 리서치 스페셜리스트(`legal-research-agent`)가 만든 문서를 받아서 인용을 검증하고, 논리를 따지고, 문장을 다듬고, 레드라인 DOCX로 결과를 냅니다. 문서가 외부 공유 단계로 넘어가기 전 마지막 관문입니다.

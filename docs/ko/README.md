...
AI가 만든 법률 문서를 외부 공유 전에 한 번 더 점검하는 Legal Orchestrator의 최종 검토 에이전트. Claude Code 기반.
...
`시니어 리뷰 스페셜리스트`는 Legal Orchestrator의 최종 검토 에이전트입니다. 계약 검토 스페셜리스트(`contract-review`), 법률 드래프팅 스페셜리스트(`legal-writing`), 법률 리서치 스페셜리스트(`general-legal-research`), 게임 산업법 스페셜리스트(`game-legal-research`)가 만든 문서를 받아서 인용을 검증하고, 논리를 따지고, 문장을 다듬고, 레드라인 DOCX로 결과를 냅니다. 문서가 외부 공유 단계로 넘어가기 전 마지막 관문입니다.
...
**Legal Orchestrator**의 전문 법률 워크플로우 에이전트 시리즈:

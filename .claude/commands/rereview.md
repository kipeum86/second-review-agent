...
2. Execute Steps RR-1 through RR-4:
   ...
   - RR-2: Delta Identification:
     1. Parse both original and revised documents using document-parser skill
        - If both parsed structures are available, run `document-parser/scripts/build-rereview-diff.py` to create the baseline mapping file
     ...
     4. Output: `working/rereview-diff.json` with `mappings[]` entries `{prior_finding_id, original_para, revised_para, mapping_status: "mapped"|"changed"|"removed"}`
